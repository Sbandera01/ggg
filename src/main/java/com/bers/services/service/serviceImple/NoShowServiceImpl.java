...
public class NoShowServiceImpl implements NoShowService {
    ...
    public void processNoShowTickets(Long tripId) {
        ...
        BigDecimal noShowFee = calculateNoShowFee();

        for (Ticket ticket : soldTickets) {
            processNoShowTicket(ticket, noShowFee);
        }

        log.info("Processed {} no-show tickets for trip {}", soldTickets.size(), tripId);
    }

    private void processNoShowTicket(Ticket ticket, BigDecimal noShowFee) {
        // Marcar ticket como NO_SHOW
        ticket.setStatus(TicketStatus.NO_SHOW);

        // Registrar el fee cobrado (esto podría generar una transacción de cobro)
        ticket.setRefundAmount(noShowFee.negate()); // Negativo porque es un cobro
    ...
    @Override
    public BigDecimal calculateNoShowFee() {
        // Obtener fee configurado o usar default
        Double feePercentage = configService.getConfigValueAsDouble("no.show.fee.percentage", 10.0);
